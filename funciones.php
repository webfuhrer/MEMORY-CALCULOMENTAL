<?php
$path = getcwd();
$rutaBBDD="sqlite:".$path."/database/database.db3";
function grabaDatosEnBD($arrayPareja1, $arrayPareja2, $id, $creador, $asignatura, $indicaciones)
	{
    
	echo ($GLOBALS['rutaBBDD']);
	$aux=maxIdBaraja();
	if($aux==$id)//Para evitar repetir id
	{echo ("ID repetido");}
	else
	{
	echo ("ID repetido");
	...
	function creaCombosBarajas()
		{
		echo ("Ruta: ".$GLOBALS['rutaBBDD']);
		$db = new PDO($GLOBALS['rutaBBDD']);//new SQLite3('database/database.db3');
